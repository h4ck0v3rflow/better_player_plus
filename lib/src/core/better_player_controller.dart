...
class BetterPlayerController {
  ...
  List<void Function(BetterPlayerEvent)?> get eventListeners =>
      _eventListeners.sublist(1);

  /// Defines a event listener where video player events will be send.
  void Function(BetterPlayerEvent)? get eventListener =>
  ...
  Future<void> _setupAsmsDataSource(BetterPlayerDataSource source) async {
    ...
    if (data != null) {
      ...
      ///Load audio tracks
      if ((betterPlayerDataSource?.useAsmsAudioTracks ?? false) &&
  ...
  void _postEvent(BetterPlayerEvent betterPlayerEvent) {
    for (final void Function(BetterPlayerEvent)? eventListener
        in _eventListeners) {
  ...
  double? getAspectRatio() =>
      _overriddenAspectRatio ?? betterPlayerConfiguration.aspectRatio;
  ...
  Future<void> _handleVideoEvent(VideoEvent event) async {
    print(
      "BetterPlayerController: _handleVideoEvent received ${event.eventType}",
    );
    switch (event.eventType) {
      ...
      case VideoEventType.tracksChanged:
        ...
        print(
          "BetterPlayerController: Processing tracksChanged event. Tracks null? ${tracks == null}",
        );
        if (tracks != null) {
          ...
          for (final track in tracks) {
            if (track is Map) {
              try {
                // Safely convert to int as platform might send doubles
                final width = (track['width'] as num?)?.toInt();
                final height = (track['height'] as num?)?.toInt();
                final bitrate = (track['bitrate'] as num?)?.toInt();
                ...
                print("BetterPlayerController: Error parsing track: $e");
              }
            }
          }
          print(
            "BetterPlayerController: Successfully parsed ${asmsTracks.length} tracks",
          );
          _betterPlayerAsmsTracks = asmsTracks;
          // Only post the public event for external listeners (e.g. quality dialog).
          // Do NOT post BetterPlayerControllerEvent.setupDataSource here — that
          // causes _BetterPlayerVideoFitWidget to set _started=false, which
          // hides the video and produces a black flash.
          _postEvent(BetterPlayerEvent(BetterPlayerEventType.setupDataSource));
        }
      default:
        break;
    }
  ...
  Future<void> stopPreCache(
    BetterPlayerDataSource betterPlayerDataSource,
  ) async => VideoPlayerController.stopPreCache(
    betterPlayerDataSource.url,
